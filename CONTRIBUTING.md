...
The four files are `test_ids.py` (id convention, 24 assertions), `test_markers.py` (activity markers, 17), `test_rotate.py` (orientation transpose, 19) and `test_roundtrip.py` (yaml to bpmn to yaml is lossless, 8). They are plain scripts with no test-runner dependency, so `PYTHONPATH=src python3` is enough and `uv run` is not required; each prints a count and exits non-zero if any of its cases failed.
